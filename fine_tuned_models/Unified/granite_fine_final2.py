...
def _load_audio_torchaudio(source, target_sr=16000):
    """Load audio from a file path or BytesIO, return mono float32 numpy array at target_sr."""
    import soundfile as sf
    waveform, sr = sf.read(source, dtype="float32", always_2d=False)
    # Mix down to mono: soundfile returns (samples, channels) for multichannel
    if waveform.ndim > 1:
    ...
    if sr != target_sr:
        ...
        waveform = resampler(wav_tensor).squeeze(0).numpy().astype(np.float32)
...
def load_audio_from_example(ex):
    ...
    if isinstance(audio_entry, dict):
        ...
        if "bytes" in audio_entry and audio_entry["bytes"] is not None:
            audio_array = _load_audio_torchaudio(io.BytesIO(audio_entry["bytes"]))
        ...
        if "path" in audio_entry and audio_entry["path"] is not None:
            ...
            if os.path.exists(audio_path):
                audio_array = _load_audio_torchaudio(audio_path)
                return audio_array, 16000
...
trainer = Trainer(
    ...
    # compute_metrics omitted: Trainer logits are [batch, audio+text, vocab]
    # while labels are [batch, text] — shape mismatch; final eval done manually below.
)
...
base_model_raw = GraniteSpeechForConditionalGeneration.from_pretrained(
    MODEL_NAME,
